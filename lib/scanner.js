...
function Scanner () {
  ...
  function mutation (text, allow) {
    ...
    */]).shift();
    var columns = [];
    var literals = {};
    if (allow != null && allow.tableOnly && !rest.length) {
      return { table: table, columns: columns, literals: literals, where: [] }
    }
    $ = advance("cannot find key specification", re["key" /*
...
exports.mutation = function mutation (sql, allow) {
  return new Scanner().mutation(sql, allow);
}
